...
for i_country in H2forEU_settings.lst_country:
    print(i_country)

    lst_cell_country = [x for x in H2forEU_settings.lst_cell if i_country in x]
    ...
    for i_cell in lst_cell_country:
        ...
        # SET ITERATION
        lst_iteration = df_template['Iteration'][df_template['Cell']==i_cell].tolist()
        H2forEU_tools_write_gdx.fct_write_gdx_single_set('ITERATION', pd.DataFrame(lst_iteration), ['ITERATION'],H2forEU_settings.path_folder_gdx)
        ...
        for i_system in H2forEU_settings.lst_system:
            ...
            if 'PV' in lst_energy_type:
                ...
                df_res_profile_system_temp = pd.read_csv(H2forEU_settings.path_folder_res_profiles + "\\" + i_cell + "_PV_"+str(H2forEU_settings.val_reference_year)+".csv")
            ...
            if 'Onshore' in lst_energy_type:
                df_res_profile_system_temp = pd.DataFrame()
                #df_res_profile_system_temp = pd.read_csv(H2forEU_settings.path_folder_res_profiles + "\\Wind\\" + i_cell + "_Wind_130m.csv")
                df_res_profile_system_temp = pd.read_csv(H2forEU_settings.path_folder_res_profiles + "\\" + i_cell + "_Onshore_"+str(H2forEU_settings.val_reference_year)+".csv")
        ...
        df_result_temp = df_template[df_template['Cell']==i_cell].copy()
        ...
        df_result_country.to_csv('C:\\Users\\Johannes\\Documents\\PhD\\06Research\\Paper2\\Tools\\LCOH\\Results_LCOH_'+i_country+'.csv', index=False)
...
df_result.to_csv('C:\\Users\\Johannes\\Documents\\PhD\\06Research\\Paper2\\Tools\\LCOH\\Results_LCOH.csv', index=False)
end_time = time.time()
print(end_time - start_time)
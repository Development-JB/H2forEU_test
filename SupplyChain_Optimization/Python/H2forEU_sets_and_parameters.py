...
def COUNTRY_IMPORT():
    ...
    df_gdx = pd.DataFrame(H2forEU_settings.lst_node_import, columns=['Node_import'])
    df_gdx['Country'] = df_gdx['Node_import'].str[:3]

    df_gdx = df_gdx[['Country']]
    df_gdx.columns = ['COUNTRY']

    gdx_name = 'COUNTRY_IMPORT'
    gdx_dimensions = df_gdx.columns.tolist()
    gdx_data = df_gdx
...
def COUNTRY_EXPORT():
    ...
    df_gdx = pd.DataFrame(H2forEU_settings.lst_node_export, columns=['Node_export'])
    df_gdx['Country'] = df_gdx['Node_export'].str[:3]

    df_gdx = df_gdx[['Country']]
    df_gdx.columns = ['COUNTRY']

    gdx_name = 'COUNTRY_EXPORT'
    gdx_dimensions = df_gdx.columns.tolist()
    gdx_data = df_gdx
...
def LINK_REGION_COUNTRY():
    ...
    df_gdx = pd.read_excel(H2forEU_settings.val_path_settings, sheet_name='Link_region_country')
    df_gdx = df_gdx[['Region','Country']]
    df_gdx.columns = ['REGION','COUNTRY']

    gdx_name = 'LINK_REGION_COUNTRY'
    gdx_dimensions = df_gdx.columns.tolist()
    gdx_data = df_gdx
...
def p_production_limit_country():
    ...
    df_gdx = H2forEU_settings.df_country_capacity
...
def p_demand_eu():
    ...
    df_gdx = pd.read_excel(H2forEU_settings.val_path_data_general, sheet_name='Demand_eu')
    df_gdx = df_gdx[df_gdx['Scenario']== H2forEU_settings.val_demand_scenario]
    df_gdx = df_gdx.set_index(['Scenario','Unit'])
    ...
    df_gdx = df_gdx[['Year','Value']]
    df_gdx.columns = ['YEAR','Value']

    gdx_name = 'p_demand_eu'
    gdx_dimensions = df_gdx.columns[: -1].tolist()
    gdx_data = df_gdx
...
def p_production_limit_volume_node():
    ...
    gdx_name = 'p_production_limit_volume_node'
    gdx_dimensions = df_gdx.columns[: -1].tolist()
    gdx_data = df_gdx
...
def p_production_limit_capacity_node():
    ...
    df_country_capacity = H2forEU_settings.df_country_capacity
    df_country_capacity = df_country_capacity.groupby(['Year','Country']).agg({'Value':'sum'}).reset_index()
    df_country_capacity = df_country_capacity.rename(columns={'Value':'Capacity_country'})

    df_area_node = H2forEU_settings.df_area_node
    df_area_node['Country'] = df_area_node['Node_production'].str[:3]

    df_area_country = df_area_node.groupby(['Country']).agg({'Area_available':'sum'}).reset_index()
    df_area_country = df_area_country.rename(columns={'Area_available':'Area_available_country'})

    df_gdx = pd.merge(df_area_node,df_area_country, how='left', on=['Country'])
    df_gdx = pd.merge(df_gdx, df_country_capacity, how='left', on=['Country'])

    df_gdx['Capacity_node'] = df_gdx['Area_available']/df_gdx['Area_available_country']*df_gdx['Capacity_country']
    df_gdx = df_gdx.rename(columns={'Capacity_node':'Value'})

    df_gdx = df_gdx[['Year','Node_production','Value']]
    df_gdx.columns = ['YEAR','NODE_PRODUCTION','Value']

    gdx_name = 'p_production_limit_capacity_node'
    gdx_dimensions = df_gdx.columns[: -1].tolist()
    gdx_data = df_gdx
...
def p_production_area_available_node():
    ...
    df_gdx = H2forEU_settings.df_area_node
    df_gdx = df_gdx[df_gdx['Node_production'].isin(H2forEU_settings.lst_node_production)]
...
def p_production_volume():
    ...
    df_gdx = H2forEU_settings.df_production.copy()
    df_gdx = df_gdx[df_gdx['Year'].isin(H2forEU_settings.lst_year)]

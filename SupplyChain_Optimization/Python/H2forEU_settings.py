...
df_country_capacity = pd.read_excel(val_path_data_general, sheet_name='Prod_limit_country')
df_country_capacity = df_country_capacity.drop(['Unit'], axis=1).set_index(['Country', 'Energy_type']).interpolate(axis=1).stack().reset_index(name='Value').rename(columns={'level_2': 'Year'})
df_country_capacity = df_country_capacity[df_country_capacity['Year'].isin(lst_year)]

df_area_node = pd.read_excel(val_path_data_general, sheet_name='Prod_area_avail_node')
df_area_node = df_area_node.drop(['Unit'], axis=1)

df_production_vres = pd.read_excel(val_path_data_general, sheet_name='Prod_data_vres')
df_production_other = pd.read_excel(val_path_data_general, sheet_name='Prod_data_other')
df_production = df_production_other.append(df_production_vres)
...
df_link_source_node_production = df_production[['Node_production']].drop_duplicates()
df_link_source_node_production_temp_res = df_link_source_node_production[~df_link_source_node_production['Node_production'].str.contains('_CB|_CX')]
...
df_link_source_node_production['Source'] = np.where(df_link_source_node_production['Node_production'].str.contains('_CB'),'Biomass',df_link_source_node_production['Source'])
df_link_source_node_production['Source'] = np.where(df_link_source_node_production['Node_production'].str.contains('_CX'),'Gas',df_link_source_node_production['Source'])
